...
fn current_text_area_selection_render_context() -> Option<TextAreaSelectionRenderContext> {
...
impl Text {
    ...
    pub fn local_char_to_screen_position(&self, local_char: usize) -> Option<(f32, f32, f32)> {
        ...
        for run in buffer.layout_runs() {
            ...
            for glyph in run.glyphs.iter() {
                if glyph.start >= target_byte {
                    return Some((
                        self.layout_position.x + glyph.x,
        ...
        if had_run {
            return Some((
                self.layout_position.x + last_x,
                self.layout_position.y + last_top,
                line_height,
            ));
        }
        None
    ...
    pub fn local_selection_screen_rects(&self, local_start: usize, local_end: usize) -> Vec<Rect> {
        ...
        selection_rects_in_buffer(buffer, self.content.as_str(), start, end)
            .into_iter()
            .map(|rect| Rect {
                x: self.layout_position.x + rect.x,
                y: self.layout_position.y + rect.y,
                width: rect.width,
                height: rect.height,
            })
            .collect()
    ...
    pub fn screen_position_to_local_char(&self, x: f32, y: f32) -> Option<usize> {
        ...
        let buffer = self.layout_buffer.as_ref()?;
        let local_x = x - self.layout_position.x;
        let local_y = y - self.layout_position.y;
        let bounds_w = self.layout_size.width.max(0.0);
        let bounds_h = self.layout_size.height.max(0.0);
