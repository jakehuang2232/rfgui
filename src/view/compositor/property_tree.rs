...
use crate::view::base_component::{
    ...
    SpatialPositionReferenceSnapshot,
...
impl PartialEq for VisualOffsetNodeSnapshot {
    fn eq(&self, other: &Self) -> bool {
        self.id == other.id
            && self.owner == other.owner
            && self.parent == other.parent
            && self.offset.to_array().map(f32::to_bits)
                == other.offset.to_array().map(f32::to_bits)
            && self.generation == other.generation
    }
}
...
impl PropertyTrees {
    ...
    fn refresh_derived_spatial_projections(&mut self) {
        ...
        let positions = self
            ...
            .map(|(&id, node)| LayoutPositionNodeSnapshot {
                ...
                reference_scroll: node.reference_scroll,
                translation_at_scroll_zero: node.translation_at_scroll_zero,
                child_reference_offset_at_scroll_zero: node
                    .child_reference_offset_at_scroll_zero,
                generation: node.generation,
            })
            .collect::<Vec<_>>();
        ...
        let graph = match SpatialProjectionGraph::try_new(
            &transforms,
            &positions,
            &visuals,
            &scrolls,
    ...
    fn sync_spatial_nodes(
        ...
        let parent = arena.parent_of(key).and_then(|parent| {
            #[cfg(test)]
            if !self.visual_offsets.contains_key(&VisualOffsetNodeId(parent)) {
                return None;
            }
