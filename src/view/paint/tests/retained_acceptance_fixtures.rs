//! Shared geometry and absolute pixel expectations for native and browser acceptance.
//! Neither renderer supplies the other renderer's expected values.
use super::*;
use crate::style::{AnchorName, Angle, Rotate, Scale, TransformOrigin};
use crate::view::test_support::get_element_mut;

pub(super) const EXTENT: [u32; 2] = [160, 128];
pub(super) const RED: [u8; 4] = [255, 0, 0, 255];
pub(super) const BLUE: [u8; 4] = [0, 0, 255, 255];
pub(super) const GREEN: [u8; 4] = [0, 255, 0, 255];
pub(super) const CLEAR: [u8; 4] = [0; 4];
...
pub(super) struct Fixture {
    ...
    pub(super) paint_owners: Vec<NodeKey>,
    pub(super) probes: Vec<(&'static str, [u32; 2], [u8; 4])>,
}

pub(super) fn style(size: [f32; 2], at: Option<[f32; 2]>, color: Option<[u8; 4]>) -> Style {
...
pub(super) fn element(id: u64, size: [f32; 2], style: Style) -> Element {
    let mut element = Element::new_with_id(id, 0.0, 0.0, size[0], size[1]);
    element.apply_style(style);
    element
}

pub(super) fn unlaid_out_fixture(scene: Scene) -> Fixture {
