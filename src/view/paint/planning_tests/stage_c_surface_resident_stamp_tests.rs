...
#[test]
fn localized_command_identity_excludes_global_paint_revisions_but_tracks_payload() {
    let artifact = scroll_artifact_with_a_local_clip();
    let baseline = prepared_depth_three(artifact.clone());
    let boundary = baseline.residents().ordered_entries()[0]
        .stamp()
        .identity
        .boundary_root;
    assert!(artifact.chunks.iter().any(|chunk| chunk.owner != boundary));
    let stamps = |frame: &crate::view::paint::PreparedArtifactSurfaceFrame| {
        sealed_stamps(frame.residents())
            .cloned()
            .collect::<Vec<_>>()
    };
    let mut revised = artifact.clone();
    for chunk in &mut revised.chunks {
        chunk.content_revision.self_paint_revision += 100;
        chunk.content_revision.composite_revision += 100;
    }
    assert_eq!(
        stamps(&baseline),
        stamps(&prepared_depth_three(revised)),
        "global revisions alone cannot invalidate identical localized commands"
    );

    let mut painted = artifact.clone();
    let chunk = painted
        .chunks
        .iter_mut()
        .find(|chunk| {
            chunk.id.slot != crate::view::paint::RETAINED_CHILD_MASK_SLOT
                && painted.ops[chunk.op_range.clone()]
                    .iter()
                    .any(|op| matches!(op, PaintOp::DrawRect(_)))
        })
        .expect("paintable rectangle");
    for op in &mut painted.ops[chunk.op_range.clone()] {
        if let PaintOp::DrawRect(rect) = op {
            rect.params.fill_color[0] = 0.75;
            rect.params.fill_color[1] = 0.25;
            break;
        }
    }
    chunk.payload_identity = chunk
        .payload_identity
        .rebuild_from_localized_ops(&painted.ops[chunk.op_range.clone()])
        .expect("complete changed payload identity");
    assert_ne!(
        stamps(&baseline),
        stamps(&prepared_depth_three(painted)),
        "changed commands invalidate raster even with unchanged generation counters"
    );
    ...
    missing
        .chunks
        .iter_mut()
        .find(|chunk| chunk.owner != boundary)
        .unwrap()
        .content_revision
        .self_paint_revision = 0;
    let plan = prepare_artifact_surface_raster_plan(missing, raster_context())
        .expect("recorded geometry still prepares");
    assert!(
        seal_prepared_artifact_surface_frame(plan).is_err(),
        "omitting global revisions from equality does not permit missing input evidence"
    );
}
