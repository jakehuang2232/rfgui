    ...
    match text_area_witness {
        ...
        Some(witness) => {
            ...
            let text_area_roles = artifact
                ...
                .collect::<Vec<_>>();
            let grammar_matches = match witness.paint_source() {
...
pub(crate) struct TransformEffectScrollChildRasterDependency {
    ...
    pub(crate) parent_opaque_order_before: u32,
    pub(crate) parent_opaque_order_after: u32,
    pub(crate) same_owner_role: Option<SameOwnerTransformEffectScrollRasterRoleStamp>,
}

/// Compiler-only role seal for one native owner carrying T, E and S. Matrix,
/// transform generation, opacity and effect generation remain composite-only;
/// this stamp freezes only role identity and stable ownership.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct SameOwnerTransformEffectScrollRasterRoleStamp {
    pub(crate) owner: crate::view::node_arena::NodeKey,
    pub(crate) stable_id: u64,
...
impl SameOwnerTransformEffectScrollRasterRoleStamp {
    fn is_canonical_for_roles(
        &self,
        outer_transform: TransformNodeId,
        child_contract: &EffectPropertySurfaceArtifactContract,
        inner_role: &SameOwnerEffectScrollRasterRoleStamp,
    ) -> bool {
        self.owner == outer_transform.0
            && self.owner == child_contract.boundary_root()
            && self.owner == inner_role.owner
            && self.stable_id != 0
            && self.stable_id == child_contract.stable_id()
            && self.stable_id == inner_role.stable_id
            && self.transform == outer_transform
            && self.transform.0 == self.owner
            && self.effect == child_contract.isolated_leaf().id
            && self.effect == inner_role.effect
            && self.effect.0 == self.owner
            && self.scroll == inner_role.scroll
            && self.scroll.0 == self.owner
            && self.contents_clip == inner_role.contents_clip
            && self.contents_clip.owner == self.owner
            && self.contents_clip.role == ClipNodeRole::ContentsClip
            && self.content_root == inner_role.content_root
            && self.content_root != self.owner
            && self.content_stable_id != 0
            && self.content_stable_id == inner_role.content_stable_id
    }
}
...
pub(crate) fn transform_effect_scroll_child_dependency_validates_contract(
    ...
    let opacity = f32::from_bits(dependency.child_opacity_bits);
    let inner_same_owner_role = dependency
        .child_stamp
        .ordered_steps
        .iter()
        .find_map(|step| match step {
            RetainedSurfaceRasterStepStamp::EffectScrollBoundary(inner) => {
                inner.same_owner_role.as_ref()
            }
            _ => None,
        });
    let same_owner_roles_are_canonical =
        match (dependency.same_owner_role.as_ref(), inner_same_owner_role) {
            (None, None) => true,
            (Some(role), Some(inner_role)) => {
                role.is_canonical_for_roles(outer_transform, child_contract, inner_role)
            }
            _ => false,
        };
    same_owner_roles_are_canonical
...
/// Compiler-sealed, graph-inert transaction for one generic no-scroll
/// Transform/Effect boundary forest. The forest node id and role are part of
/// the stamp authority; callers cannot reinterpret an Effect resident as a
/// Transform resident even when both boundaries share one owner.
#[derive(Clone, Debug, PartialEq, Eq)]
pub(crate) struct RetainedPropertyBoundaryForestTransactionStamp {
    forest: super::frame_plan::PropertyBoundaryForest,
    ordered_stamps: Vec<RetainedSurfaceRasterStamp>,
}

impl RetainedPropertyBoundaryForestTransactionStamp {
    pub(super) fn new(
        forest: super::frame_plan::PropertyBoundaryForest,
        ordered_stamps: &[RetainedSurfaceRasterStamp],
    ) -> Option<Self> {
        property_boundary_forest_transaction_is_canonical(&forest, ordered_stamps).then(|| Self {
            forest,
            ordered_stamps: ordered_stamps.to_vec(),
        })
    }
    ...
    pub(crate) fn new_for_test(
        ...
        Self::new(forest, ordered_stamps)
    }

    pub(crate) fn is_canonical(&self) -> bool {
        property_boundary_forest_transaction_is_canonical(&self.forest, &self.ordered_stamps)
    }

    pub(crate) fn validates_ordered_stamps(
        &self,
        ordered_stamps: &[RetainedSurfaceRasterStamp],
    ) -> bool {
        self.is_canonical() && self.ordered_stamps == ordered_stamps
    }

    pub(crate) fn validates_forest_and_ordered_stamps(
        &self,
        forest: &super::frame_plan::PropertyBoundaryForest,
        ordered_stamps: &[RetainedSurfaceRasterStamp],
    ) -> bool {
        self.is_canonical() && self.forest == *forest && self.ordered_stamps == ordered_stamps
    }
...
pub(crate) enum ArtifactSurfaceCompositeGeometryStamp {
    Transform {
        ...
        resolved_receiver_clip: ResolvedClip,
    },
    Effect {
        source_bounds_bits: [u32; 4],
        destination_bounds_bits: [u32; 4],
...
impl ArtifactSurfaceCompositeGeometryStamp {
    ...
    pub(crate) fn resolved_receiver_clip(self) -> ResolvedClip {
        match self {
            Self::Transform {
...
pub(crate) fn validated_scroll_text_area_content_raster_stamp(
    ...
    artifact_span: RetainedSurfaceArtifactSpanStamp,
    aggregate_opaque_order_span: Range<u32>,
    paint_source: super::PaintTextContentSource,
) -> Option<RetainedSurfaceRasterStamp> {
    ...
    let text_area_chunks = semantic
        ...
        .cloned()
        .collect::<Vec<_>>();
    let grammar_matches = match paint_source {
        super::PaintTextContentSource::Glyphs => {
...
pub(crate) fn validated_scroll_interactive_text_area_content_raster_stamp(
    ...
    let source = resident.paint_source();
    let (_, semantic) = classify_optional_child_mask_stamp_semantics(
        artifact_span.chunks.as_slice(),
        boundary_root,
    )?;
    ...
    if !resident.is_canonical_for(source) || !resident_matches {
        return None;
    }
    validated_retained_surface_tree_raster_stamp_with_scroll(
        boundary_root,
...
fn property_boundary_forest_transform_geometry_is_canonical(
    geometry: &RetainedSurfaceCompositeGeometryStamp,
    child: &RetainedSurfaceRasterStamp,
) -> bool {
    let RetainedSurfaceCompositeGeometryStamp::Transform {
...
fn property_boundary_forest_role_for_stamp(
    stamp: &RetainedSurfaceRasterStamp,
) -> Option<super::frame_plan::PropertyBoundaryForestRole> {
    match stamp.identity.role {
        RetainedSurfaceRasterRole::Transform => {
            Some(super::frame_plan::PropertyBoundaryForestRole::Transform)
        }
        RetainedSurfaceRasterRole::PropertyEffect => {
            Some(super::frame_plan::PropertyBoundaryForestRole::Effect)
        }
        RetainedSurfaceRasterRole::RootIsolation
        | RetainedSurfaceRasterRole::NestedIsolation
        | RetainedSurfaceRasterRole::ScrollHost
...
fn property_boundary_forest_leaf_stamp(
    stamp: &RetainedSurfaceRasterStamp,
    span: Option<&RetainedSurfaceArtifactSpanStamp>,
) -> Option<RetainedSurfaceRasterStamp> {
    let (owner_topology, clip_nodes, chunks, op_count, opaque_order_span, ordered_steps) =
        match span {
            Some(span) => {
                let end = span
                    .opaque_order_span
                ...
                normalized.step_index = 0;
                normalized.opaque_order_span = 0..end;
                (
                    span.owner_topology.clone(),
                    span.clip_nodes.clone(),
                    span.chunks.clone(),
                    span.op_count,
                    0..end,
                    vec![RetainedSurfaceRasterStepStamp::ArtifactSpan(normalized)],
                )
            }
            None => (Vec::new(), Vec::new(), Vec::new(), 0, 0..0, Vec::new()),
        };
    let mut leaf = stamp.clone();
    leaf.owner_topology = owner_topology;
    leaf.clip_nodes = clip_nodes;
    leaf.chunks = chunks;
    leaf.op_count = op_count;
    leaf.opaque_order_span = opaque_order_span;
    leaf.ordered_steps = ordered_steps;
    Some(leaf)
}

/// Generic no-scroll T/E surface canonicalizer. Child role, rather than a
/// whole-scene grammar enum, selects both composite geometry and cursor
/// semantics. This is intentionally graph-inert until the executor adopts the
/// matching forest transaction token.
pub(crate) fn property_boundary_forest_surface_stamp_is_canonical_at_depth(
    stamp: &RetainedSurfaceRasterStamp,
    initial_depth: usize,
) -> bool {
    fn validate(
        stamp: &RetainedSurfaceRasterStamp,
        depth: usize,
        resident_keys: &mut FxHashSet<RetainedSurfaceResidentKey>,
        role_owners: &mut FxHashSet<(
            super::frame_plan::PropertyBoundaryForestRole,
            crate::view::node_arena::NodeKey,
        )>,
    ) -> bool {
        let Some(role) = property_boundary_forest_role_for_stamp(stamp) else {
            return false;
        };
        let Some(identity_leaf) = property_boundary_forest_leaf_stamp(stamp, None) else {
            return false;
        };
        let identity_is_canonical = match role {
            super::frame_plan::PropertyBoundaryForestRole::Transform => {
                property_scene_surface_raster_stamp_is_canonical_at_depth(&identity_leaf, depth)
            }
            super::frame_plan::PropertyBoundaryForestRole::Effect => {
                property_effect_surface_raster_stamp_is_canonical_at_depth(&identity_leaf, depth)
            }
        };
        if depth >= usize::from(u8::MAX)
            || !identity_is_canonical
            || !resident_keys.insert(stamp.identity.resident_key())
            || !role_owners.insert((role, stamp.identity.boundary_root))
        {
            return false;
        }
        ...
        for (expected_index, step) in stamp.ordered_steps.iter().enumerate() {
            match step {
                RetainedSurfaceRasterStepStamp::ArtifactSpan(span) => {
                    let Some(leaf) = property_boundary_forest_leaf_stamp(stamp, Some(span)) else {
                        return false;
                    };
                    let span_is_canonical = match role {
                        super::frame_plan::PropertyBoundaryForestRole::Transform => {
                            property_scene_surface_raster_stamp_is_canonical_at_depth(&leaf, depth)
                        }
                        super::frame_plan::PropertyBoundaryForestRole::Effect => {
                            property_effect_surface_raster_stamp_is_canonical_at_depth(&leaf, depth)
                        }
                    };
                    if span.step_index != expected_index
                        || span.opaque_order_span.start != cursor
                ...
                RetainedSurfaceRasterStepStamp::NestedSurface(dependency) => {
                    let Some(child_role) =
                        property_boundary_forest_role_for_stamp(&dependency.child_stamp)
                    else {
                        return false;
                    };
                    let geometry_is_canonical = match child_role {
                        super::frame_plan::PropertyBoundaryForestRole::Transform => {
                            property_boundary_forest_transform_geometry_is_canonical(
                                &dependency.child_composite_geometry,
                                &dependency.child_stamp,
                            )
                        }
                    ...
                    let expected_after = match child_role {
                        super::frame_plan::PropertyBoundaryForestRole::Transform => {
                            cursor.max(dependency.child_stamp.opaque_order_span.end)
                        }
                        super::frame_plan::PropertyBoundaryForestRole::Effect => cursor,
                    };
                    if dependency.parent_opaque_order_after != expected_after {
                        return false;
                    }
                    cursor = expected_after;
                }
...
#[allow(clippy::too_many_arguments)]
pub(crate) fn validated_property_boundary_forest_surface_raster_stamp(
    boundary_root: crate::view::node_arena::NodeKey,
    stable_id: u64,
    color_key: crate::view::frame_graph::PersistentTextureKey,
    role: RetainedSurfaceRasterRole,
    ...
    ordered_steps: Vec<RetainedSurfaceRasterStepStamp>,
    aggregate_opaque_order_span: Range<u32>,
    property_effect: Option<PropertyEffectRasterIdentityInputs>,
) -> Option<RetainedSurfaceRasterStamp> {
    if stable_id == 0
        || aggregate_opaque_order_span.start != 0
        || !matches!(
            role,
            RetainedSurfaceRasterRole::Transform | RetainedSurfaceRasterRole::PropertyEffect
        )
    {
        return None;
    }
    ...
    for (expected_index, step) in ordered_steps.iter().enumerate() {
        match step {
            ...
            RetainedSurfaceRasterStepStamp::NestedSurface(dependency) => {
                let child_role = property_boundary_forest_role_for_stamp(&dependency.child_stamp)?;
                let expected_after = match child_role {
                    super::frame_plan::PropertyBoundaryForestRole::Transform => {
                        cursor.max(dependency.child_stamp.opaque_order_span.end)
                    }
...
fn property_boundary_forest_transaction_is_canonical(
    forest: &super::frame_plan::PropertyBoundaryForest,
    ordered_stamps: &[RetainedSurfaceRasterStamp],
) -> bool {
    ...
    if !forest.is_structurally_canonical()
        || forest.nodes.len() != ordered_stamps.len()
        || ordered_stamps.is_empty()
    {
        return false;
    }
    let mut depths = Vec::<usize>::with_capacity(forest.nodes.len());
    let mut by_role_owner = FxHashMap::default();
    let mut resident_keys = FxHashSet::default();
    for (ordinal, (node, stamp)) in forest.nodes.iter().zip(ordered_stamps).enumerate() {
        let expected_raster_role = match node.stable_key.role {
            Role::Transform => RetainedSurfaceRasterRole::Transform,
            Role::Effect => RetainedSurfaceRasterRole::PropertyEffect,
        };
        let depth = match node.receiver {
            Receiver::FrameRoot { .. } => 0,
            Receiver::Surface { parent, .. } => {
                let Some(parent_depth) = depths.get(parent.0 as usize) else {
                    return false;
                };
        ...
        if node.id.0 as usize != ordinal
            || stamp.identity.role != expected_raster_role
    ...
    for (parent_ordinal, parent_stamp) in ordered_stamps.iter().enumerate() {
        for step in &parent_stamp.ordered_steps {
            ...
            let Some(child_role) = property_boundary_forest_role_for_stamp(&dependency.child_stamp)
            else {
                return false;
            };
            let Some(&child_ordinal) =
                by_role_owner.get(&(child_role, dependency.child_stamp.identity.boundary_root))
            else {
                return false;
            };
            if !matches!(
...
impl RetainedAtomicProjectionTextAreaResidentRasterSeal {
    ...
    pub(crate) fn is_canonical(&self) -> bool {
        self.artifact_source.is_canonical_for(self.text_area_root)
...
pub(crate) fn validate_scroll_scene_text_area_content_artifact(
    ...
    let selection_matches = |selection: &super::PaintChunk| {
        let Some(source) = paint_source.selection() else {
            return false;
        };
            ...
            && ops.iter().all(|op| {
                matches!(
                    ...
                    PaintOp::DrawRect(rect)
                        if rect.params.fill_color.map(f32::to_bits) == source.color_rgba_bits
...
pub(super) fn validate_scroll_scene_atomic_projection_text_area_content_artifact_parts(
    ...
    let artifact_source = raster_oracle.artifact_source().clone();
    ...
    let validated = validate_artifact_store_with_policy(
        ...
        ArtifactStoreValidationPolicy::ScrollSceneAtomicProjectionTextAreaContent {
            content_root,
            text_area_root,
            projection_text_root: artifact_source.projection_text_owner,
            contents_clip: contents_clip.id,
        },
    );
    ...
    if !matches!(validated.target, ValidatedArtifactTarget::CurrentTarget)
        ...
        || !glyph_exact(
            projection_glyph,
            projection_ops,
            artifact_source.projection_text_owner,
...
fn validate_scroll_scene_atomic_projection_selection_text_area_content_artifact_parts(
    ...
    let artifact_space_transition = raster_oracle.artifact_space_transition();
    let artifact_source = raster_oracle.artifact_source().clone();
    let selection_source = raster_oracle.selection_source();
    let start_char = selection_source.start_char;
    let end_char = selection_source.end_char;
    let color_rgba_bits = selection_source.color_rgba_bits;
    let [contents_clip] = raster_oracle.clip_nodes() else {
        return None;
    };
    let contents_clip = *contents_clip;
...
pub(super) fn validate_scroll_scene_atomic_projection_text_area_plan_parts(
    ...
    let artifact_source = local_raster_oracle.artifact_source().clone();
    ...
    if !resident.is_canonical()
        ...
        || resident.artifact_source != artifact_source
...
pub(super) fn validate_scroll_scene_atomic_projection_selection_text_area_plan_parts(
    ...
    let artifact_space_transition = local_raster_oracle.artifact_space_transition();
    let artifact_source = local_raster_oracle.artifact_source().clone();
    let selection_source = local_raster_oracle.selection_source();
    let start_char = selection_source.start_char;
    let end_char = selection_source.end_char;
    let color_rgba_bits = selection_source.color_rgba_bits;
    let boundary_root = outer_scroll.owner;
...
pub(crate) fn validate_scroll_scene_interactive_text_area_content_artifact(
    ...
    let selection_resident = |selection: &super::PaintChunk| {
        let source = paint_source.selection()?;
        let start_char = source.start_char;
        let end_char = source.end_char;
        let color_rgba_bits = source.color_rgba_bits;
    ...
    let resident = match paint_source {
        super::PaintTextContentSource::Glyphs => {
    ...
    if !resident.is_canonical_for(paint_source)
