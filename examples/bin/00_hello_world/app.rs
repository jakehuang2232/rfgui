...
impl App {
    ...
    fn render_frame(&mut self) {
        ...
        if let Some(app) = &self.app {
            self.with_viewport_mut(|viewport| {
                let promotion_config = viewport.promotion_config();
    ...
    fn finalize_frame_updates(&mut self) -> bool {
        ...
        // clipboard_write handled in phase 5 backend; currently no-op.
    ...
    fn sync_theme_visuals(&mut self) {
        self.applied_theme_dark = Some(true);
        self.background_color = app_background_color(true);
        self.with_viewport_mut(|viewport| {
